...
remDisplay::remDisplay(Users* user, set<Users>& users, QWidget* parent) : QWidget(parent) {
    ...
    if (!user->dateMp.empty()) {
        ...
        tableWidget->setHorizontalHeaderLabels(QStringList() << "Reminder at" << "Name" << "Place" << "Start Date" << "End Date" << "Description");
        ...
        for (auto it = user->reminderMp.begin(); it != user->reminderMp.end(); it++) {
            ...
            QTableWidgetItem* Item5 = new QTableWidgetItem(QString::fromStdString(it->second.description));
